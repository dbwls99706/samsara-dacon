...
## 최신 (2026-06-03, D-5 산출물, 29회차) — 파괴 prop 잔존 사망 버그 + 운석 위협화 + 발톱 교차 X 🐯

사용자 라이브 3건: ①"포탄 부쉈는데 뭔가 남아서 죽음" ②"포탄이 위협적으로 안 생김, 우리 팀 같아" ③"할퀴기가 앞으로 뻗지 말고 교차로 샥샥 대각선+대각선".

### ① 🔴 파괴된 prop 접촉 데미지 잔존 사망 (`game/world.ts tickProps`)
- 진단: 파괴된 prop(destroyedAt)은 **1초 파괴 애니** 동안 world.props 에 남음(cleanup L1379). 그동안 tickProps 가 상호작용(asteroid 접촉 dmg 1/800ms·blackhole 인력)을 계속 돌려 "부쉈는데 뭔가 남아서 죽음".
- fix: tickProps 루프 최상단에 `if (p.destroyedAt) { >1000ms 면 splice; continue }` — 파괴 즉시 모든 상호작용 중단. 하단 cleanup 의 destroyedAt 분기는 위로 이동(stardust consumed 분기만 잔류). **보상은 destroyProp 에서 동기 지급 완료라 추가 틱 불필요.**
- 🧪 회귀 2종: 파괴된 asteroid 접촉 0 피해 + 정상 asteroid 는 피해(causation).

### ② 운석(asteroid) 위협 비주얼 — "우리 팀처럼 안 보이게" (`render/world.ts`)
- 보라 크리스탈(#b14aff)→**적색 위협 운석**: 적색 언더글로우 펄스 + 삐죽 8각 바위(정점 반경 흔듦) + 달궈진 주황/흰 코어 + 적색 림. [[feedback_threat_readability]] 색-팀(빨강=위협). 라벨 ◆포탄→**☄충돌(danger 적색)**. 파괴 ring 색도 적색. (접촉=피해 hazard 이므로 위협 신호가 맞음.)

### ③ 🔴 부수효과 발견: 기도 중 shrine 자동파괴 (Pray Mode 불가) — 잠복 버그
- ①의 올바른 가드가 노출: 자동 발톱(11dmg/0.7s)이 shrine(hp50) ~2.8s 에 파괴 → 기도(3.0s) 직전 죽어 Pray Mode 사실상 불가. (이전엔 파괴된 shrine 도 기도 완료되던 quirk 가 마스킹.)
- fix: `isPropWeaponImmune(p)` export(`shrine && prayProgress>0`) → 발톱 부채꼴/체인/발사체/노바 4 데미지 경로에서 기도 중 shrine 면역. 파괴↔기도 상호배타(Stag Hunt) 설계와 일치. 멀어지면 prayProgress decay→0 으로 다시 파괴 가능. 🧪 회귀 1종.

### ④ 발톱(claw) 교차 X 모션 (`render/attacks.ts`)
- 28회차 매직클로(앞으로 발사) 폐기 → **교차 X 대각선 2연속**(↘ 먼저 k0~0.55, ↗ 나중 k0.30~1.0, 겹침구간 X 형성). 각 대각선=3-prong 발톱+선두 흰 스파크. **range 80 유지**(제자리 rake). 격리 하니스로 X read 눈검증.
- ⚠ 영구: **claw = 교차 X 대각선 rake** (27 제자리긁기·28 매직클로 모두 폐기, 이게 최종).

### 검증 (fresh 실측)
- `npm run build` PASS 워닝 0 / gzip **117.03 KB** / `npm test` **211 PASS / 15 files**(+3 회귀). 커밋 **129f062** 푸시.

---

## (2026-06-03, D-5 산출물, 28회차) — 할퀴기→메이플 매직클로 + 특수기물 역할 라벨 🐯

사용자: "할퀴기가 메이플의 **매직클로**처럼 되라고… 그리고 특수 기물들 더 무슨 역할인지 잘 눈에 띄게 개편." (27회차의 '제자리 샥샥 긁기'를 뒤집음 — 매직클로는 캐릭터에서 발사되는 에너지볼트라 forward-projecting 이 정답.)

### ① 할퀴기 → 매직클로 (`render/attacks.ts` case 'claw')
- 두 갈래 **V-스프레드 발톱 에너지볼트**(각 3-prong quadratic curve), `travel=min(1,k*1.5)` 로 startR→endR 앞으로 전진, 선두 **밝은 흰 팁** dot, k>0.5 부터 fade-out. **range 80 유지**(범위 불변)·duration 0.3. 빠르고 펀치감.

### ② 특수기물 역할 라벨 (`render/world.ts` prop 루프 ~L1176)
- 추상 아이콘만으론 역할 인지 불가 → 머리 위 **아이콘+한글 역할어 pill**. 10종: ★보상 ♥체력 ✦부스트 ◆포탄 ⚠흡입 ❄강화 ◉함정 ☼코인↑ ◈반사 ☠저주.
- 위험 3종(blackhole 흡입 / pressure_plate 함정 / cursed_totem 저주) = **적색 계열**(색-팀 규칙 빨강=위협, [[feedback_threat_readability]]). 보더+글로우(펄스)+꼬리 tick 으로 기물 지시. `!p.consumed` 게이트. `const c=ctx!` 별칭으로 rr() 클로저 null-내로잉.

### 검증 방법 (라이브 캡처 부적합 → 격리 하니스)
- 봇이 14s 사망 + 특수기물 500~2700px 산개 + claw FX 0.3s → 주기 캡처로 둘 다 잡기 어려움. **격리 HTML 하니스**에 실제 claw 지오메트리·라벨 코드 verbatim 복제 → file:// Playwright 샷. 매직클로 read(k=0.1→0.95 스냅샷) + 라벨 10종 가독 눈검증 후 temp 전부 정리.
- ⚠ 영구: **claw = forward-projecting 매직클로** (27회차 제자리긁기 폐기). 기물 라벨 = prop 루프 내 한정.

### 검증 (fresh 실측)
- `npm run build` PASS 워닝 0 / gzip **116.79 KB** / `npm test` **208 PASS / 15 files**
- 커밋 **6bbad88** 푸시 완료.

### ⚠ 사용자 잔여 (변동 없음)
- 보스 난이도(W5≈1056, W25≈22.4k)·xpForNext 35 체감 + YouTube + DACON 제출폼 + 6/8 라이브 재배포.

---

## (2026-06-03, D-5 산출물, 27회차) — 할퀴기 모션 + 부활 윤회스왑 버그 수정(타에이전트 오검증 규명) 🐯

사용자 요청 ①할퀴기 모션 ②(이어서) 타에이전트(Codex) 다수 편집분 검토. **타에이전트가 "검증 완료"라 보고했으나 부활 스왑이 실제로는 작동 안 함을 코드 추적으로 규명** → 사용자 지시(제대로 고치고 한 번에 커밋)대로 수정.

### ① 할퀴기(starter_claw) 모션 — '발사'→'샥샥 긁기' (`render/attacks.ts`)
- 진단: claw FX 가 `lenK=sin(kπ)` 로 startR·endR 둘 다 방사형으로 **자라며 뻗어** = 캐릭터에서 발사되는 듯. 사용자 "캐릭터에서 나가는 게 아니라 샥샥 긁는 모션".
- fix: 방사 성장 제거 → **공격 호를 가로지르는 X자 이중 스와이프**(dir ±1, tilt ±0.16, 시간차). startR 고정·endR=range(범위 유지). spawn duration 0.25→0.3. 캡처(cw-05) = 발톱 3선이 적 방향 짧은 rake, 다음 프레임 사라짐 = 제자리 긁기 확인.

### ② 🔴 부활 윤회 스왑 버그 수정 — 타에이전트 "검증" 주장이 거짓이던 것
- 타에이전트가 `state.ts handleLifeLoss` 에 "부활 시 무작위 카드 1장 교체"(이전에 사용자가 거부했던 고위험안)를 구현 + walkthrough.md 에 "검증 완료" 기록. **코드 추적 결과 2중 버그**:
  - 🔴 **무기 안 바뀜**: `main.ts:880` subscribeState 무기재구축이 `cards.length` 게이트 → 교체는 개수 불변이라 **rebuildWeapons 미발동**. "환생 시 무기가 바뀐다" 의도가 화면에 0 반영(배너만 뜸). → **`lastCardSig`(카드 id 시그니처) 게이트로 교체** ([[feedback_subscribestate_tdz]] 준수).
  - 🔴 **효과 미적용**: `recalcAfterCardMutation`(cards.ts)은 시너지/RI **배너만** 갱신, `runEffectsImmediate`/`ri.bonus` 미실행. → `state.ts` 부활 블록에 handlePickCard 와 동일한 effect 적용 인라인 + 미사용 import 제거.
  - 🧪 회귀 테스트 1종(`improvements.test.ts`): 부활 시 카드 개수 불변·내용(id sig) 변경 단언 → length 게이트 버그 재발 가드. **207→208**.
- ⚠ 교훈: **타에이전트 "검증 완료" 보고도 file:line 직접 추적 필수** (Antigravity 이력과 동일 패턴). build/test PASS ≠ 기능 동작.

### 그 외 타에이전트 편집분 (검토 후 보존 — 사용자 승인분)
- 보스 **대폭 상향**: base 300→**400**, kindHpMult divine 4.0→**8.0**/mega 2.2→**3.5**/normal→1.2, scale 0.16→**0.24**. → W5≈1056, W25 divine≈**22,400 HP**(이전 6,000). ⚠ 매우 공격적 — 실플레이 체감 확인 필요(과하면 하향).
- xpForNext 16→**35**(주석 stale 정정). ⚠ 첫 카드가 ~30초 첫보상 윈도 넘길 수 있음 — 사용자 이전 "완만(16)" 선택과 상충, 체감 확인 권장.
- tokens.css 6색 15~20% 톤다운(브랜드 hex 변경 — 이전 "유지" 선택 뒤집음), 카드픽 "획득/강화!" 배너, 무기 라벨 강화(11px·불투명↑·max-width 110), 카운트다운 줌단계 가드(`_zoomingToCardPick`), 우측 prop 페이드(rightBand 160).

### 검증 (fresh 실측)
- `npm run build` PASS 워닝 0 / gzip **116.56 KB** / `npm test` **208 PASS / 15 files** / `verify-all` ALL GATES PASS(console 0/0, a11y clean, e2e/impression/lighthouse ✓)

### ⚠ 사용자 확인 권장
1. **보스 난이도**(W5≈1056, W25≈22.4k) 실플레이 체감 — 8.0× divine 과할 수 있음. 2. **xpForNext 35** 첫보상 타이밍(30초 넘는지). 3. YouTube 영상 + DACON 제출폼 + 라이브 재배포(6/8).

---

## (2026-06-03, D-5 산출물, 26회차) — 라이브 피드백 6종: 보스/윤회연출/카운트다운/색피로/무기명/성취감 🎮

사용자 라이브 플레이 후 6건 제보 + 외부 에이전트(Codex) 계획서 제출. **계획서를 그대로 승인하지 않고** 6피드백은 전부 타당하나 *해결책* 일부가 D-5(동결 D-2)에 고위험이라 판단 → 3개 고위험 항목을 사용자에게 AskUserQuestion → **전부 안전안 선택**. 실측·캡처 눈검증 후 구현·커밋(`e27a8a6`)·푸시.

### 실측 후 의사결정 (추측 금지)
- 보스 HP 200 flat, kind(mega/divine) HP 미반영 / xpForNext 10×1.4(첫카드 ~10초) / countdown 시작시점만 모달체크 — 3건 코드로 확인 후 진행.
- 🔴 **반대한 계획**: ①부활 시 무기 무작위 교체(빌드 파괴 불쾌+회귀위험) ②6 브랜드 토큰 일괄 채도하향(카드/시너지/색맹팔레트 전체 번짐, 브랜드 변경) ③xpForNext 10→35(첫보상 30초 깨짐) ④"획득완료" 중앙배너(#3/#4와 자가당착). → 사용자가 안전안 3개 선택.

### 구현 6종
- **#1 보스** (`world.ts`/`boss.ts`/`main.ts`/`gameRuntime.ts`): base 300 + `kindHpMult`(normal1/mega2.2/divine4.0, **기존 0 반영이던 것**) + wave스케일 0.1→0.16. ⚠ 봇이 W1서 죽어 W5보스 '느낌'은 **사용자 수동 플레이 검증 필요**(헤드리스 한계).
- **#2 윤회 연출** (`render/world.ts`+`main.ts`+`state.ts`): 시각·프레이밍 선택. 웨이브 GO + 부활 시 supernova 버스트 + 스프라이트 화이트 램프인(`triggerRebirthFlash`, REBIRTH_FLASH_MS 520). 부활배너 'REVIVE'→'윤회·還生'. **게임플레이 변수 0**(메커니즘 미변경). reducedMotion 게이트.
- **#3 카운트다운** (`main.ts countdown step()`): 매 스텝 `getScreen()!=='play' || #levelup-modal` 재검사→즉시 host.remove()+countdownEndsAt=0. 시작시점만 보던 갭(진행 중 카드픽 위로 3-2-1) 차단.
- **#4 색 피로** (`terrain.ts BIOME_TINT`+`render/world.ts`): 브랜드 hex 유지, **배경 앰비언트만** 감쇠 — biome틴트 ~40%↓(cursed0.24→0.15 등), elevation명도 ±22→±13%, 글리프×0.6, 먼지40→26. gameplay 액터/위협 가독성 불변. 캡처: 액터가 calm 배경 위 도드라짐.
- **#5 무기명 상시** (`main.ts renderWeaponHud`): teach(초반14초)만 보이던 이름 라벨 → **상시**. 평상시 9px 옅은 글래스 pill(#4 클러터 균형). 캡처: W3 생존 시 5무기 전부 이름 노출.
- **#6 성취감** (`world.ts`): xpForNext 10→16(첫카드 ~30초 = '30초 첫보상' 정렬) + 배율 1.4→1.46(후반 가파르게). 캡처 XP "1/23" 확인.

### 검증 (fresh 실측)
- `npm run build` PASS 워닝 0 / gzip **116.15 KB** / `npm test` **207 PASS / 15 files** / `verify-all` ALL GATES PASS(console 0/0, a11y clean, e2e/impression/lighthouse ✓)
- ⚠ impression `load_ms<3000` 게이트는 콜드 프리뷰 스타트에 flaky(7.6s→재실행 1.9s). 내 변경(JS +0.6KB, 새 에셋0)과 무관.
- 시각: rebirth-go(GO supernova 버스트), labels-survive(W3 5무기명 상시), 배경 calm 전부 눈검증.

### ⚠ 사용자만 할 수 있는 잔여
1. **보스 난이도 '느낌' 수동 검증** (W5 첫 보스가 묵직한지 vs 지루한지 — 헤드리스 봇 불가). 과하면 kindHpMult/base 하향.
2. YouTube 시연영상 + DACON 제출폼 3종 URL (6/8 마감) 3. 라이브 재배포(이 변경 반영) 4. iOS 실기기.

---

## (2026-06-03, D-5 산출물, 25회차) — 모바일 인게임 디클러터: HUD 축소 + 장식 prop HUD밴드 페이드 🧹

이전 세션(Codex)이 모바일 가독성/클러터 분석 후 main.ts 에 HUD 축소(①②)를 미커밋으로 남겨둠. 인계받아 **캡처로 눈 검증**(추측 금지) 후 ③ 마무리 + 전체 커밋(`9b6265c`).

### ① ② HUD 축소·재배치 + 무기 라벨 컴팩트 (이전 Codex 분, 검증 후 유지 — `main.ts`)
- `@media(max-width:520px)`: 미니맵 176→124px·상향(top118), 무기레일 축소(아이콘 46→42, gap↓), 점수/시간/WAVE 폰트 반응형 축소, 콜아웃 top92, 입력힌트 줄바꿈. 데스크톱 불변.
- 초반 무기 이름 라벨 노출 조건 `wave<=2` → **`wave<=1 && elapsed<14`** (FTUE 교육만, 액션화면 덜 가림). 라벨 자체도 컴팩트(폰트 11→10, max-width 90px ellipsis).

### ③ 장식 prop HUD 세이프존 페이드 (이번 세션 신규 — `render/world.ts drawWorld`)
- 진단(캡처 눈검증): 하단 회색 바위/잔해 prop 이 XP바·스탯 HUD 위로 **선명히 깔려 "UI 밑 버려진 잔해"** 클러터. (HUD 프레임 배경이 rgba 0.25~0.55 반투명 + 하단 중앙은 HUD 요소 없는 빈 영역이라 prop 그대로 비침.)
- fix: prop 루프에서 화면 Y 기준 **하단 밴드(132px)·상단 칩 밴드(104px)** 안의 prop 본체+그림자를 부드럽게 페이드(바닥 0.32~0.40, 완전소멸 X → 위치 인지 가능, 경계 팝 방지 램프). `drawDecoration` 가 `ctx.save()` 로 globalAlpha 상속 → 본체 페이드. 그림자 `0.40*hudFade`.
- **gameplay 액터(적/픽업/플레이어/투사체)는 이 루프 밖이라 0 영향** — 색-팀 위협 가독성([[feedback_threat_readability]]) 보존. hazard 텔레그래프(블랙홀/등불/별먼지 글로우)는 각자 절대 alpha 라 fade 무관 = 경고 풀 밝기 유지. 매 iteration `globalAlpha=1` 리셋(누수 방지).

### 검증 (fresh 실측)
- `npm run build` PASS 워닝 0 / gzip **115.93 KB** / `npm test` **207 PASS / 15 files** / `verify-all` ALL GATES PASS(console 0/0, a11y clean, e2e/impression/lighthouse ✓)
- 시각: 모바일 포트레이트(412×915) 재캡처 = 하단 debris 클러스터 흐릿하게 가라앉고 XP/CARDS 위로 깔끔. 가로모드(844×390) 레이아웃 온전 + 적/젬 페이드 안 됨 확인.

### ⚠ 사용자만 할 수 있는 잔여 (변동 없음)
1. **YouTube 시연 영상 녹화/업로드** (6/8 마감, 미제출 시 실격) 2. DACON 제출폼 3종 URL 기입 3. iOS 실기기 오디오/safe-area 4. 라이브 재배포(이 디클러터 반영) — Vercel.

---

## (2026-06-02, D-6 산출물, 24회차) — WAVE 박스 jitter fix + 임프레션 거짓경고 규명 🎯

사용자 라이브 제보 2건. 둘 다 **추측 금지, 코드+좌표/probe 직접 검증** 후 처리. (외부 에이전트 보고는 file:line 검증 — 과거 오진 이력.)

### 🔴 ① 상단 가운데 WAVE 박스가 초마다 좌우로 흔들림 (진짜 버그 — `main.ts` hud-top)
- 원인: `justify-content:space-between` + 좌(점수)/우(TIME) 칩이 `flex:0 1 auto`(내용 가변폭). TIME 숫자가 100ms마다 `30.0→9.9→…`로 폭이 바뀌니 space-between 분배가 재계산돼 **가운데 WAVE 칩 중심이 매 프레임 드리프트**. 메모리 [[feedback_hud_chip_overflow]] 패턴.
- fix: WAVE 칩을 flex 흐름에서 빼 **절대 중앙 고정**(`position:absolute;left:50%;top:12px;transform:translateX(-50%);z-index:1`). 좌/우 칩은 각자 자기 변(좌·우)에 앵커되어 더 이상 가운데를 못 민다. + `#hud-time`/`#hud-score` 에 `font-variant-numeric:tabular-nums`(자릿수 폭 고정, 숫자 시머 제거). **min-width 안 씀**([[feedback_hud_chip_overflow]] 모바일 우측 칩 잘림 회피).
- 검증(결정적 probe): WAVE 박스 중심 x = **206px(=412/2) 6샘플 전부 고정, spread 0px**. TIME 26.6→18.4 변동에도 미동 0.

### 🟢 ② "32초에 카드픽 미등장" 임프레션 경고 = 거짓경고 규명 (Antigravity 보고, 게임 버그 아님)
- Antigravity가 "웨이브 타이머/state 전환 점검 필요"라 진단 → **오진 확정**. 2중 검증:
  1. **하네스 모순**: `e2e-impression.mjs` 루프 윈도 `TAP_TIMEOUT_MS=32_000` 인데 판정 기준 `promise_first_card_pick_under_35s`는 35초. 디자인 범위도 "30~35초". 33초 카드픽(정상)을 32초 루프가 관측 불가 → **윈도 38s로 상향** + 루프 직후 최종 체크 추가.
  2. **타이머 정상 확정(probe)**: `#hud-time` 샘플링 = 정상 부하에서 게임타임이 실시간과 **1:1 전진**(25.7→17.3 over 8.4s, ratio 1.00). 41초까지 안 끝난 건 임프레션 테스트의 무거운 입력(마우스 드래그 8스텝×4방향+매스텝 탭)이 헤드리스 fps를 20 미만으로 떨궈 **dt 클램프(`main.ts:1754` `Math.min(0.05,…)`)로 게임타임이 늘어진 자기유발 아티팩트**. 실기기 60fps 무관.
- fix: `judge_impression` 메시지를 사실대로(30초+ 생존=정상, 카드픽 미관측=헤드리스 dt클램프 탓, 타이머 결함 아님)로 교체 → 미래 에이전트 phantom 추적 방지.

### 🟢 ③ 카드픽 더블클릭 디바운스 (Antigravity 적용분, working tree에 있던 것 — 검증 후 유지)
- `screens.ts mountCardPick` 카드선택/리롤/스킵 핸들러에 `clicked` 플래그 가드. 빠른 더블/트리플 클릭 시 `startWave()` 중복 호출로 wave 2→3 동시가동 방지. 리롤은 재mount(새 클로저)라 기능 손상 0. 엔진 가드(`state.ts:268`)는 2차 방어로 유지. **검증 후 타당 → 유지.**

### 검증 (fresh 실측)
- `npm run build` PASS 워닝 0 / gzip **115.56 KB** / `npm test` **207 PASS / 15 files** / `verify-all` ALL GATES PASS(console 0/0, a11y clean, e2e-impression 이제 ✓)
- ⚠ 미커밋이던 RESUME(23회차) + screens.ts(디바운스)도 이번 커밋에 함께 포함.

---

## (2026-06-01, D-7 산출물, 23회차) — 제출 전 최종 검토 ✅

사용자 "제출 전 최종 검토". 추측 금지, **실제 게이트 + 라이브 URL 직접 검증** 후 발견된 신뢰 갭만 fix.

### ✅ 제출 3종 실측 검증 (6/8 10:00 마감)
1. **배포 URL** — `https://samsara-dacon.vercel.app` HTTP **200**, 타이틀 정확. 🎯 라이브 JS 해시(`index-CF7noXJm.js`)가 fresh 로컬 빌드와 **완전 동일** = 22·22-2회차 critical fix(무방비 사망 버그) 전부 라이브 반영 확인. OG 썸네일 PNG도 HTTP 200(276KB). **실격 #1(URL live) 해소.**
2. **GitHub** — origin/main 동기화, CI 최신 런(4d83b1c) green(1m51s).
3. **YouTube 시연 영상** — ⚠ 사용자만 가능, 미확인 (제출 전 필수).

### 🔴 발견·수정 — README stale 수치 6곳 (신뢰 직격, commit 0153ea1)
- README가 "npm test 로 직접 검증 가능, 과장 0" 약속(1차 60%=동료 개발자) ↔ 본문은 stale: 테스트 **196**(실제 207) 3곳 + 번들 **113KB**(실제 115) + CI 게이트 lighthouse 누락 2곳. 동료가 `npm test` 한 번이면 잡는 모순.
- fix: 6곳 실측값 동기화 → 커밋·푸시(0153ea1). README 변경은 JS 해시 불변 → 라이브 게임 무영향.
- ⚠ 21회차에 "196→207 고침"이라 적혔으나 본문 3곳이 안 고쳐진 채였음 — **수치 변경 시 grep 전수 확인**.

### 검증 (fresh 실측)
- `npm run build` PASS 워닝 0 / gzip **115.50 KB** / `npm test` **207 PASS / 15 files** / `verify-all` ALL PASS(console 0/0, a11y clean, FCP OK)

### ⚠ 사용자만 할 수 있는 잔여 (제출 전 필수)
1. **YouTube 시연 영상 녹화/업로드** (콘티 = `docs/11_demo_video.md` 75초).
2. **DACON 제출폼에 3종 URL 기입** (배포/GitHub/YouTube).
3. (선택) iOS 실기기 오디오 — 헤드리스 검증 불가.
4. (선택) README 라인 284 "Opus 4.7" → 4.8 또는 버전 생략 (AI 크레딧 자체는 정직한 공개라 유지 권장, 버전만 stale).

---

## (2026-05-31, D-8 산출물, 22회차) — 라이브 플레이 5종 버그 fix 🐛

사용자가 실제로 플레이하며 발견한 5건. 전부 코드 fix + 캡처/게이트 검증 + 커밋(e151048)·푸시 완료.

1. **젬↔적 구분 안 됨** → XP 젬을 흰 테두리 밝은 시안 크리스털로 재렌더(`render/world.ts` case 'xp'). 부드러운 빨강 적 블롭과 명확 대비 — 캡처 검증(play-gem.png).
2. **장애물 피격 판정 이상 + 적 없을 때 우선타격** → `nearestTarget`(world.ts)는 이미 거리기반(적·prop 혼합)이라 적 없으면 prop 우선 OK. 추가로 발견: **일반 prop 파괴 sweep 부재** → 직접 hp 데미지가 좀비 prop 남김. `tickWorld`에 `applyWeapons` 직후 hp<=0 prop 파괴 sweep 추가(world.ts:1682).
3. **튜토 제목 줄바꿈 안 됨 + 배경 글리프 과대** → stepTitle `word-break:keep-all;overflow-wrap:break-word`, stepGlyph 워터마크를 우하단 작게(clamp 34~64px, alpha 0.07)로 축소(`screens.ts`). 캡처 검증(tutorial-p1.png).
4. **카드선택 중 3-2-1 카운트다운이 화면 가림** → `countdown()`이 카드선택/레벨업 모달(`#levelup-modal`) 활성 중이면 120ms 후 재시도해 표시 보류(`main.ts`). "카드 선택 중엔 시간 정지" 구현.
5. **호랑이 발톱(starter_claw)으로 오브젝트 피격 안 됨** → makeClaw에 props 부채꼴 타격 루프 추가(`weapons.ts`, `DESTRUCTIBLE_KINDS` import) + 위 2번 sweep으로 실제 파괴까지 연결. 적보다 살짝 넓은 부채꼴(π/2.5).

### 검증
- `npm run build` PASS / gzip **115.48 KB** / `npm test` **207 PASS**(props_game_theory·world 포함) / `verify-all` ALL PASS(console 0/0)
- 커밋 e151048 + dev/QA 헬퍼 스크립트 6종 동봉. **Claude 흔적 없이** 커밋(사용자 영구 지시).

### 사용자만 할 수 있는 것 (변동 없음 — 아래 21회차 ⚠ 참조)
1. 배포 URL LIVE 확인 / 2. 시연 영상 녹화 / 3. iOS 실기기 오디오 / 4. OG 캐시 갱신(카카오톡·페북).

---

## (2026-05-30, D-9 산출물, 21회차) — 1등 푸시: 제출물 전달력 (시연 콘티 + OG 썸네일 + README) 🏆

사용자 "1등하게 해". 분석상 코드 한계효용 낮음 → 최대 레버 = **전달(영상·참신성 부각·첫인상)**. 코드 밖 자산 정비:

### 🔴 OG 소셜 미리보기 썸네일 — SVG→PNG (실작동 fix)
- index.html OG 메타는 완비됐으나 `og:image` 가 **SVG** → 카카오톡·디스코드·트위터 등 **대부분 플랫폼이 SVG OG 미렌더 = 썸네일 안 뜸**(1차 투표 첫인상 손해).
- fix: og-image.svg → Playwright 로 **1200×630 PNG 렌더**(public/og-image.png, 한글·이모지 정상), index.html og:image/twitter:image → .png + og:image:type. dist 포함 확인.

### 🔴 시연 영상 정밀 콘티 (docs/11_demo_video.md)
- 기존 문서는 "만드는 법"만 있고 샷 단위 콘티 부재 → **75초 1등 최적화 콘티** 작성. 첫 8초 윤회 훅(약축 참신성) + 궁극/시너지 타격감(약축 재미) + 윤회 계승 명시 컷 + 5평가축 의도 배치.

### 🟡 README 신뢰 정비
- stale 수치 196→**207 테스트**, 113→115KB. (README "과장0 검증가능" 약속 ↔ 실제 npm test 불일치 = 신뢰 훼손 방지.) 참신성 줄을 "윤회 계승" 훅으로 강화.

### ⚠ 사용자만 할 수 있는 1등 결정 작업 (코드 밖)
1. **배포 URL 실제 LIVE 확인** (README "배포 예정" 표기 잔존 — 실격 #1 리스크). 6/8 후 수정=심사제외.
2. **시연 영상 녹화/편집** (위 콘티대로 — 투표·심사 첫인상 지배).
3. **iOS 실기기 오디오 확인** (19회차 P0 fix 실엔진 미검증).

---

## (2026-05-30, D-9 산출물, 20회차) — 접근성/i18n 직접 점검 (Antigravity 한도 도달, 내가 인계) ♿

Antigravity 가 a11y/i18n "계획서"(implementation_plan.md)만 쓰고 한도 도달 — **게임 코드는 0 구현**(src/ 미변경, untracked 헬퍼 스크립트 6개만 남김). 내가 인계해 직접 점검.

### 핵심 발견: 인프라 대부분 이미 완비 (계획서 전제가 틀림 — 코드 안 보고 작성)
- **색맹 모드**: `colorblindMode` + 데미지 숫자에 모양(●○◆◇★) + 식별 팔레트 + 시스템 고대비 자동감지 — 이미 구현·작동(캡처 검증). 색-단독 의존 안 함.
- **모션 최소화**: `reducedMotion` 셰이크/플래시/파티클/히트스톱 전부 게이트 + prefers-reduced-motion 자동감지 + tokens.css 미디어쿼리 — 완비.
- **i18n**: `i18n.ts` t()/setLang()/onLangChange + 언어 토글(한/영) + **ko/en 112키 완전 1:1 페어링(누락 0)** — 완비.
- **포커스**: 전역 `:focus-visible` 아웃라인(components.css:391) + .btn/.card-big — 완비. 버튼 aria(verify-all 강제).

### ✅ 실제로 한 것 (진짜 빈 곳)
- 설정 토글(makeToggle)에 **`role=switch` + `aria-label` + `aria-checked`** 추가 — 스크린리더가 "무엇을/현재상태" 인지. 유일하게 비어있던 a11y 항목.
- 색맹 모드 라이브 캡처 검증(samsara-colorblind 적용·적/보상 구분 유지).

### ⏸ 의도적 보류 (D-9 고위험/저가치 — 사용자 판단 시 재개)
- **텍스트 스케일(largeText)**: 코드가 px/clamp/vw 인라인(rem/CSS변수 아님)이라 retrofit 고난도+방금 튜닝한 반응형 레이아웃과 `!important` 충돌 위험. 웹은 브라우저 줌(Ctrl/핀치)이 이미 텍스트 확대 제공 → 커스텀 largeText 가치 낮음. **스킵 권장.**
- **전면 i18n 리팩터(하드코딩 한국어 → t())**: 설정 라벨 등 일부 하드코딩 잔존하나, 한국 심사위원이 한국어로 플레이 → EN 커버리지 저가치 + 고위험. 112키는 이미 페어링됨. **스킵 권장.**

### 검증
- `npm run build` PASS / gzip **115.31 KB** / `npm test` **207 PASS** / `verify-all` ALL PASS(a11y_clean)
- ⚠ untracked: Antigravity 헬퍼 스크립트 6종(auto-sim/balance-calc/test-ultimates/viewport_* 등) — 게임 무관, 미커밋.

---

## (2026-05-30, D-9 산출물, 19회차) — Antigravity 크로스디바이스 감사: iOS 오디오/가로모드/Safari 블러 📱

3 항목 전부 코드 검증 → 진짜. 처리:

### 🔴 P0 iOS Safari 오디오 영구 무음 위험 (`audio/sfx.ts`, `main.ts`)
- 원인: `unlockAudio()` 가 화면 라우터 구독(main.ts:466)에서만 호출 = 제스처 콜스택 동기성 미보장 + 전역 첫-제스처 리스너 없음. iOS 는 `resume()` 가 제스처 안에서 실행돼야 함.
- fix: ①`unlockAudio` 에 iOS 무음버퍼 프라임 추가(resume 만으론 일부 iOS suspended 복귀). ②`window` 에 첫 입력(pointerdown/touchend/mousedown/keydown, capture+once) 전역 리스너로 즉시 동기 언락. 멱등이라 기존 경로와 충돌 X.

### 🔴 P1 가로모드 무기레일 잘림 (`main.ts <style>`)
- `#hud-weapons top:316px` 세로컬럼이 landscape(height 390)에서 화면 밖. → `@media(max-height:500px)`: 미니맵 축소(100px)+하향(top80, TIME칩 겹침 회피), 레일 가로 wrap 행(top188, max-width228). 실측: railClipped false, timeMiniOverlap false.

### 🟡 P2 WebKit backdrop-filter 누락 (`main.ts`/`screens.ts`/`errorBoundary.ts`)
- backdrop-filter 34곳 중 webkit 접두사 2곳뿐 → 구형 Safari 블러 미적용(iOS16.4+ 는 unprefixed OK). → 스크립트로 31곳 `-webkit-backdrop-filter` 안전 추가(lookbehind 로 중복 방지).

### 검증
- `npm run build` PASS / gzip **115.25 KB** / `npm test` **207 PASS** / `verify-all` ALL PASS(console 0/0)
- 가로모드(844×390) 재캡처: TIME 온전·미니맵·무기레일 무잘림.
- ⚠ 실 iOS Safari/Android 는 헤드리스로 검증 불가 → 사용자 실기기 수동 확인 항목(오디오 첫터치 언락, 노치 safe-area, 100vh 주소창).

---

## (2026-05-30, D-9 산출물, 18회차) — 카드픽/카운트다운 플로우 버그 + 게임필 오디오 배선 🔊
