...
export function createWorld(): World {
  return {
    ...
    cameraZoomTarget: 1,
    xp: 0,
    xpForNext: 35, // 첫 카드 요구 XP — 광속 레벨업(기존 10) 방지로 성취감 페이싱(#6). ⚠ 35는 첫 카드가 ~30초 첫보상(CLAUDE §6.4) 윈도를 넘길 수 있어 실플레이 체감 확인 권장.
    level: 1,
    pendingLevelUps: 0,
    props: generateProps(),
    boostUntil: 0,
...
const ENEMY_DEF: Record<EnemyKind, Omit<Enemy, 'id' | 'pos' | 'vel' | 'spawnTime' | 'slow' | 'slowUntil' | 'burning' | 'burnUntil' | 'hitFlashUntil' | 'spawning' | 'attackCd' | 'chargeUntil' | 'chargeDir' | 'exploded' | 'summonCd'>> = {
  ...
  boss:      { kind: 'boss',      hp: 400, hpMax: 400, speed: 50, radius: 60, damage: 50, coinDrop: 500, xpDrop: 100, color: '#ff2a6d' },
};
...
export function spawnBoss(world: World, t: number, hpMult: number = 1, kind: BossKind = 'normal'): Enemy {
  ...
  const kindHpMult = kind === 'divine' ? 8.0 : kind === 'mega' ? 3.5 : 1.2;
